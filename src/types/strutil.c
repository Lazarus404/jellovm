...
static void jello_fatal(const char* msg) {
...
double utod(const uchar* str, uchar** end) {
	...
	// - copies up to JELLO_UTOD_MAX_PREFIX code units that look like a numeric prefix
	...
	char buf[JELLO_UTOD_MAX_PREFIX + 1];
	char* w = buf;
	const char* wend = buf + JELLO_UTOD_MAX_PREFIX;
	const uchar* p = str;
	while(w < wend) {
...
int utoi(const uchar* str, uchar** end) {
	...
	// - parse up to JELLO_UTOI_MAX_DIGITS code units total (including sign if present)
	...
	int64_t acc = 0;
	int ndigits = 0;
	while(consumed < JELLO_UTOI_MAX_DIGITS) {
		unsigned int c = (unsigned int)(*p);
...
int uvszprintf(uchar* out, int out_size, const uchar* fmt, va_list arglist) {
	...
	while(out < end) {
		...
		if(spec == (uchar)'.' || (spec >= (uchar)'0' && spec <= (uchar)'9')) {
			jello_fatal("Unsupported printf format");
		}
...
void uprintf(const uchar* fmt, const uchar* str) {
	...
	char fmt_stack[JELLO_UPRINTF_STACK_BUF];
	char str_stack[JELLO_UPRINTF_STACK_BUF];
