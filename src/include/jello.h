...
static inline int jello_bound_this_is_set(jello_value v) {
  return v != 0 && !jello_is_null(v);
}
...
/* KINDOF / SWITCH_KIND kind codes — stable for compiler match lowering.
   Keep in sync with `jelloc/src/jlo/format.rs` (KindOf). */
enum {
  JELLO_KINDOF_NULL     = 0,
  JELLO_KINDOF_BOOL     = 1,
  JELLO_KINDOF_I32      = 2,  /* i8/i16/i32 all box to i32 in Dynamic */
  JELLO_KINDOF_ATOM     = 3,
  JELLO_KINDOF_BYTES    = 4,
  JELLO_KINDOF_FUNCTION = 5,
  JELLO_KINDOF_LIST     = 6,
  JELLO_KINDOF_ARRAY    = 7,
  JELLO_KINDOF_OBJECT   = 8,
  JELLO_KINDOF_ABSTRACT = 9,
  JELLO_KINDOF_BOX_I64  = 10,
  JELLO_KINDOF_BOX_F64  = 11,
  JELLO_KINDOF_BOX_F32  = 12,
  JELLO_KINDOF_BOX_F16  = 13,
  JELLO_KINDOF_ENUM     = 14,
  JELLO_KINDOF_UNKNOWN  = 0x7fffffffu,
};

#define JELLO_KINDOF_OBJ_BASE 3u
#define JELLO_KINDOF_FROM_OBJ(obj_kind) (JELLO_KINDOF_OBJ_BASE + (uint32_t)(obj_kind))

/* --- bytecode.h --- */
#define JELLO_BC_MAGIC 0x4A4C5942u

typedef enum jello_bc_feature_flags {
  JELLO_BC_FEAT_NONE = 0,              // No features
  JELLO_BC_FEAT_CONST64 = 1u << 0,     // 64-bit constants
  JELLO_BC_FEAT_CONSTBYTES = 1u << 1,  // Bytes constants
  JELLO_BC_FEAT_CAP_START = 1u << 2,   // Capture start
  JELLO_BC_FEAT_LINE_TABLE = 1u << 3,  // Per-insn source line table
  JELLO_BC_FEAT_LINE_COL = 1u << 4,    // Line table entries are u32 line/col packs
...
typedef enum jello_op {
  ...
  JOP_OBJ_GET = 124,
  JOP_OBJ_SET = 125,
  JOP_ARRAY_CONCAT2 = 126,
  JOP_ARRAY_RESIZE = 127,
  JOP_OBJ_HAS = 128,
  JOP_OBJ_REMOVE = 129,
  JOP_OBJ_CLEAR = 130,
  JOP_OBJ_COPY = 131,
  ...
  JOP_BITAND_I32 = 136,
  JOP_BITAND_I64 = 137,
  JOP_BYTES_BITAND2 = 138,
  JOP_BITOR_I32 = 139,
  JOP_BITOR_I64 = 140,
  JOP_BITXOR_I32 = 141,
  JOP_BITXOR_I64 = 142,
  JOP_BYTES_BITOR2 = 143,
  JOP_BYTES_BITXOR2 = 144,
  JOP_BYTES_READ_U16_LE = 145,
  JOP_BYTES_READ_U16_BE = 146,
  JOP_BYTES_READ_U32_LE = 147,
  JOP_BYTES_READ_U32_BE = 148,
  JOP_BYTES_READ_I32_LE = 149,
  JOP_BYTES_READ_I32_BE = 150,
  JOP_BYTES_READ_F32_LE = 151,
  JOP_BYTES_READ_F32_BE = 152,
  JOP_BYTES_WRITE_U16_LE = 153,
  JOP_BYTES_WRITE_U16_BE = 154,
  JOP_BYTES_WRITE_U32_LE = 155,
  JOP_BYTES_WRITE_U32_BE = 156,
  JOP_BYTES_WRITE_I32_LE = 157,
...
/* Reserved VM ABI atoms (trap catch payloads, System.deepEqual result). */
#define JELLO_ATOM_KIND 2u
#define JELLO_ATOM_MESSAGE 3u
#define JELLO_ATOM_CODE 4u
#define JELLO_ATOM_EQUAL 5u
#define JELLO_ATOM_DIFF 6u
...
struct jello_function;
typedef struct jello_function jello_function;
...
jello_exec_status jello_vm_exec_status_chunk(jello_vm* vm, const jello_bc_module* m,
    ...
    uint32_t entry_module_idx, jello_value* out, jello_value* out_exports);
/** Run a bytecode closure (with captures) on an isolated VM. */
...
/** Print JDLL + bytecode frames for the current trap (no-op if empty). */
...
/**
 * Discovery anchor for JDLL / script-relative native search.
 * Uses `JELLO_ENTRY_PATH` when set (logical `.jello` / app entry), else `argv_entry`
 * (typically the `.jlo` bytecode path passed to jellovm).
 */
const char* jello_discovery_entry_path(const char* argv_entry);
/** Copy program argv (strings after the `.jlo` path on the CLI). Replaces any prior args. */
void jello_vm_set_program_args(jello_vm* vm, int argc, char** argv);
void jello_vm_clear_program_args(jello_vm* vm);
/** Enable or disable the partial JIT at runtime (no-op when JIT not compiled in). Default: enabled. */
void jello_vm_set_jit_enabled(jello_vm* vm, int enabled);
int jello_vm_jit_enabled(const jello_vm* vm);
/** Verify linked `.jlo` JDLL dependency paths resolve (no-op when section absent). */
int jello_jdll_preflight_module(const struct jello_bc_module* m, const char* entry_jlo_path);

/* --- threading (POSIX) --- */
typedef struct jello_thread jello_thread;
typedef struct jello_channel jello_channel;

jello_vm* jello_current_vm(void);
uint32_t jello_thread_current_id(void);
jello_thread* jello_thread_spawn_fn(jello_vm* parent, const struct jello_bc_module* m,
    jello_function* fn, const jello_value* args, uint32_t nargs);
...
int jello_array_resize(struct jello_vm* vm, const struct jello_bc_module* m, jello_array* a, uint32_t new_len);

/* --- enum.h --- */
typedef struct jello_enum {
  jello_obj_header h;
  uint32_t tag;
  uint32_t nfields;
  jello_value fields[];
...
/* --- UTF-16 helpers --- */
#define JELLO_UTOD_MAX_PREFIX 30
#define JELLO_UTOI_MAX_DIGITS 16
#define JELLO_UPRINTF_STACK_BUF 512
