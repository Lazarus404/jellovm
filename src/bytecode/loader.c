...
void jello_bc_free(jello_bc_module* m) {
  ...
  if(m->funcs) {
    for(uint32_t i = 0; i < m->nfuncs; i++) {
      ...
      free((void*)m->funcs[i].lines);
    }
  }

  if(m->source_files) {
    for(uint32_t i = 0; i < m->nsource_files; i++) {
      free((void*)m->source_files[i]);
    }
  }
...
#define JELLO_FUN_VARARGS_TAG 0x40000000u
...
static jello_bc_result validate_type_entry(const jello_type_entry* t, uint32_t ntypes, uint32_t nsigs) {
  switch(t->kind) {
    ...
    case JELLO_T_FUNCTION: {
      uint32_t sig_id = t->as.fun.sig_id & ~JELLO_FUN_VARARGS_TAG;
      if(sig_id >= nsigs) return err(JELLO_BC_BAD_FORMAT, "bad signature id", 0);
      return ok();
    }
...
jello_bc_result jello_bc_read(const uint8_t* data, size_t size, jello_bc_module** out) {
  ...
  *out = (jello_bc_module*)calloc(1, sizeof(jello_bc_module));
  ...
  if(m->features & (uint32_t)JELLO_BC_FEAT_LINE_TABLE) {
    uint32_t nsource = 0;
    rr = rd_u32(&r, &nsource);
    if(rr.err) { jello_bc_free(m); *out = NULL; return rr; }
    m->nsource_files = nsource;
    if(nsource > 0) {
      char** files = (char**)calloc((size_t)nsource, sizeof(char*));
      if(!files) { jello_bc_free(m); *out = NULL; return err(JELLO_BC_OUT_OF_MEMORY, "oom source_files", r.off); }
      for(uint32_t i = 0; i < nsource; i++) {
        ...
        if(!rd_need(&r, (size_t)len)) {
          jello_bc_free(m); *out = NULL;
          return err(JELLO_BC_EOF, "unexpected EOF in source_files", r.off);
        }
        char* s = (char*)malloc((size_t)len + 1u);
        if(!s) { jello_bc_free(m); *out = NULL; return err(JELLO_BC_OUT_OF_MEMORY, "oom source_file", r.off); }
        if(len > 0) memcpy(s, r.p + r.off, (size_t)len);
        s[len] = 0;
        r.off += (size_t)len;
        files[i] = s;
      }
      m->source_files = (const char* const*)files;
    }
  }
  ...
  if(m->nfuncs > 0) {
    ...
    for(uint32_t i = 0; i < m->nfuncs; i++) {
      ...
      uint16_t source_file = 0;
      uint32_t* lines = NULL;
      if(m->features & (uint32_t)JELLO_BC_FEAT_LINE_TABLE) {
        rr = rd_u16(&r, &source_file);
        if(rr.err) { free(reg_types); jello_bc_free(m); *out = NULL; return rr; }
        if(ninsns > 0) {
          lines = (uint32_t*)malloc(sizeof(uint32_t) * (size_t)ninsns);
          if(!lines) { free(reg_types); jello_bc_free(m); *out = NULL; return err(JELLO_BC_OUT_OF_MEMORY, "oom lines", r.off); }
          int line_col = (m->features & (uint32_t)JELLO_BC_FEAT_LINE_COL) != 0;
          for(uint32_t j = 0; j < ninsns; j++) {
            if(line_col) {
              rr = rd_u32(&r, &lines[j]);
            } else {
              uint16_t line_only = 0;
              rr = rd_u16(&r, &line_only);
              lines[j] = (uint32_t)line_only;
            }
            if(rr.err) { free(lines); free(reg_types); jello_bc_free(m); *out = NULL; return rr; }
          }
        }
      }
      ...
      funcs[i].insns = insns;
      funcs[i].source_file = source_file;
      funcs[i].lines = lines;
      funcs[i].nlines = ninsns;
    }
  }
